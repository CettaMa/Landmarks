...
def pupil_circularity(eye_points):
    ...
    area = math.pi * ((distance.euclidean(eye_points[1][0], eye_points[3][1]) * 0.5) ** 2)
    return (4 * math.pi * area) / (perimeter ** 2)
    ...
    with open(csv_path, 'w', newline='') as csv_file:
        ...
        while True:
            ...
            if frame_count % interval == 0:
                ...
                # Save original frame
                cv2.imwrite(os.path.join(output_dir, 'before', f'{timestamp:.2f}.jpg'), frame)
                ...
                if results.multi_face_landmarks:
                    for face_landmarks in results.multi_face_landmarks:
                        ...
                        # Calculate metrics
                        ear = round((calculate_aspect_ratio(right_eye) + calculate_aspect_ratio(left_eye)) / 2, 4)
                        ...
                        cv2.imwrite(os.path.join(output_dir, 'after', f'{timestamp:.2f}.jpg'), frame)
